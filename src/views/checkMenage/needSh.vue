      ...
      <el-tabs type="border-card">
        <el-tab-pane label="待审核需求">
          <el-table :data="tableData" border style="width: 100%">
            ...
            <el-table-column prop="info" label="需求内容"></el-table-column>
            <el-table-column width="180" label="审核">
              <template slot-scope="scope">
                <el-button
                  type="success"
                  @click="needshenhe(scope.row.id,1)"
                  icon="el-icon-check"
                  circle
                ></el-button>
                <el-button
                  type="danger"
                  @click="needshenhe(scope.row.id,3)"
                  icon="el-icon-delete"
                  circle
                ></el-button>
              </template>
    ...
    needListsh() {
      ...
      this.$api
        ...
        .then(res => {
          ...
          this.$store.commit("UPDATE_NEED_CHECK_NUM", this.total);
